...
**Is the gender inequality index related to adolescent birth rate?**
...
country_joined %>%
  ...
  geom_point(alpha=0.6) +
  labs(
    title= "Is Gender Inequality Index Related to Adolescent Birth Rate?",
...
```
The gender inequality index measures gender-based gaps in education, life expectancy, and incomes, and the closer the value is to 1, the more unequal a society is ([United Nations Development Programme](http://hdr.undp.org/en/content/gender-inequality-index-gii)). We see that as the index approaches 1, there is a strong, positive, linear increase (r= 0.8267) in adolescent birth rate (per 1,000 15-19-year-olds). This relationship makes sense because we assume that less equal societies wouldn't allow women to make their own reproductive choices. They are also less likely to have resources and healthcare options for women.   
...
```
> I think labor participation is the one we should choose instead of gender inequality index because the slope doesnt make sense ("every one unit increase in GNI, but isn't this on a 0-1 scale?)
...
```
# Looking closer at [the most significant indicator]
```{r}
...
```
Since the two variables with the highest correlation for teen birth rate and gender inequality index and female labour participation, we ran correlations on both to find the exact values. Female labour participation’s correlation was 0.310, indicating a weak correlation. However, the correlation for gender inequality index was 0.827. Since this variable has the strongest correlation of all our variables, this is the issue we propose the majority of funding and resources go into to help lower adolescent birth rate. 

> we did correlation for these earlier in the relationship section. Is there another type of test or distribution you could run to show the importance of gender inequality index? We just need a final BOOM
