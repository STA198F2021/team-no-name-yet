...
names(hdi5_clean) <- c("HDI Rank", "Country", "Gender Development Index", "Gender Development Index Rank", "Maternal Mortality Ratio", "Adolescent Birth Rate", "Seats Parliament Female", "Secondary Education Female", "Secondary Education Male", "Labour Participation Female", "Labour Participation Male")
...
contribute to the status of development and prosperity a country has. The questions
we will attempt to answer are: How does a country’s quality of life influence the
birth rates for teenagers aged 15 to 19 years old? In other words, how does a 
country’s human and economic resources affect the rate of premature births? Since
the repercussions of teenage pregnancy can spread into many aspects of life, we 
have chosen a few different explanatory variables to compare how these differences
...
```{r gdp-linear-regression}
gdp_linreg <- country_joined %>%
  select(`Adolescent Birth Rate`, `GDP`, `HDI Rank.x`, `GNI Per Capita`, `GDP Annual Growth`) %>%
  filter(!`GDP` == "..") %>%
  filter(!`Adolescent Birth Rate` == "..")
         
mainfit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(as.numeric(`Adolescent Birth Rate`) ~ as.numeric(`GDP`) + as.numeric(`HDI Rank.x`) + as.numeric(`GNI Per Capita`), data = gdp_linreg)
tidy(mainfit)
```
...
country_pivot <- country_joined %>%
  pivot_wider( cols = c("Life Expectancy Birth Male", "Life Expectancy Birth Female"),
    names_from = gender,
    values_from = "Adolescent Birth Rate"
  )

ggplot(country_joined, aes(x = as.numeric(`Life Expectancy Birth Male`), 
                           y = as.numeric(`Adolescent Birth Rate`))) +
  geom_point()
...
**Which variable has the strongest correlation to adolescdent birth rate?**
```{r}
linear_reg() %>% 
  set_engine("lm") %>%
