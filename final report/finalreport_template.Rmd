...
We see that the middle 50% of countries have a teenage birth rate between 13.2 and 70.5. Considering a high birth rate is 40-50 births per 1,000 women, our box plot shows a distribution of relatively high rates. We also have a few outlines, which are Chad (161.1), Mali (169.1), and Niger (186.5). All three have HDI ranks 187, 184, and 189, respectively.
...
>>We can add this to our linear regression, which we will conduct later in this report to test which variables have the most substantial influence over adolescent birth rates.

>>>I don't think this sentence is necessary tbh. ^^
>> idk i think it sets up for why we use logisitc instead of simply stating all relationships. we're testing to see if there's a relationship here so we can see significance in regression (is that even how u use regression idk)??
...
>>Thus, we can also add female educational attainment to our linear regression.

>>Again could be taken out ^^^
...
Thus, we will include 2019 HDI in our regression.  
...
# Linear regression
After looking at the relationships, we decided to run a linear regression analysis in order to determine which predictors had the most influence over teenage pregnancy rates. The general null hypothesis test for every explanatory variable is beta# = 0, which indicates that there is no difference in influence when we control for each one individually. 
